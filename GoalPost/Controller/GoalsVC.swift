...
extension GoalsVC: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let incrementAction = UITableViewRowAction(style: .normal, title: "ADD 1") { (action, indexPath) in
            self.setProgress(atIndexPath: indexPath)
            self.goalsTableView.reloadRows(at: [indexPath], with: .automatic)
        }
        incrementAction.backgroundColor = #colorLiteral(red: 0.9385011792, green: 0.7164435983, blue: 0.3331357837, alpha: 1)
